...
class FSDPStrategy(FSDPStrategyBase):
    ...
    def wrap_model(self, model: nn.Module, device_mesh: DeviceMesh) -> FSDP:
        ...
        # 模型可以挂 `_fsdp_force_lora_wrap` 强制启用 LoRA wrap policy. SmolVLA 自己
        # 跑完 PEFT 后会把 cfg.is_lora 设回 False (防止外层 get_model 二次 PEFT wrap),
        # 但仍需要 wrap_policy 给 trainable 叶子 (LoRA A/B + value_head Linear) 单独
        # wrap 成 FSDP 单元, 否则会撞 _writeback_orig_params shape-mismatch bug.
        is_lora_for_wrap = bool(
            self.cfg.model.is_lora or getattr(model, "_fsdp_force_lora_wrap", False)
        )
        auto_wrap_policy = get_fsdp_wrap_policy(
            module=model,
            config=self.cfg.fsdp_config,
            is_lora=is_lora_for_wrap,
        ...
        # 模型可以挂一个 `_fsdp_ignored_modules` 列表, 让我们把整块完全冻结的子树
        # 排除出 FSDP 的 FlatParameter. 这是绕开 PyTorch FSDP1 在 use_orig_params=True
        # 下对冻结参数的 _writeback_orig_params shape-mismatch bug 的唯一干净办法
        # (FSDP1 forward 会把 module.weight 从 Parameter 降级成 Tensor, 冻结参数
        # 不参与 backward 永远不会被恢复, 下次 forward writeback 必 raise).
