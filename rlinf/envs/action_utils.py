...
def prepare_actions_for_maniskill(
    ...
    # SO-101 走 pd_joint_pos：6 个关节角直通，无需 EE 解析。
    # 单位转换（unnormalize + deg → rad）已经在 SmolVLA wrapper 的
    # predict_action_batch 里做完，这里到达的 actions 已经是 SAPIEN 期望的弧度。
    # 这里曾经误以为 model 直接输出度数，加过 `* π/180` —— 错的，因为 LeRobot
    # 模型输出的是 NORMALIZED 域 (~N(0,1))，必须先 `* std + mean` 拿回度数，
    # 再 `* π/180` 转弧度。完整链路在 model wrapper 里处理。
    if "so101" in policy:
        return raw_chunk_actions
